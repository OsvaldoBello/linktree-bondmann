...
| DT-020 | Primeira execução real do `bench` ainda não aconteceu | P1 | O [ADR-022](#adr-022--bench-sem-sandbox-do-chrome-doc-drift-passa-a-depender-de-bench) corrigiu os dois bugs estruturais que impediam `bench` e `doc-drift` de passar, mas a correção em si nunca rodou contra o GitHub Actions — só localmente (`size-limit`, que não depende de Chrome) e via `zizmor` (que audita workflow, não execução). Os limiares do §10 (Performance ≥ 98, LCP < 1.2s etc.) foram herdados do `ADR-017` sem nenhuma medição real do Lighthouse até hoje. Acompanhar o primeiro `bench` verde deste PR; se algum limiar reprovar por medição real (não por bug de configuração), recalibrar `.lighthouserc.json` com o número real, documentando o porquê — o mesmo padrão do `ADR-017` |
